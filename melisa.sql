...
CREATE TABLE IF NOT EXISTS `melisa`.`securityGroups` (
  `id` SMALLINT NOT NULL AUTO_INCREMENT,
  `idSecurityGroupType` SMALLINT NOT NULL,
  `idIdentityCreated` VARCHAR(36) NOT NULL,
  `name` VARCHAR(75) NOT NULL,
  `active` TINYINT NOT NULL DEFAULT 1,
  `createdAt` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  ...
  `updatedAt` DATETIME NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_securityGroups_securityGroupsTypes1_idx` (`idSecurityGroupType` ASC),
  INDEX `fk_securityGroups_identities1_idx` (`idIdentityCreated` ASC),
  CONSTRAINT `fk_securityGroups_securityGroupsTypes1`
    FOREIGN KEY (`idSecurityGroupType`)
    REFERENCES `melisa`.`securityGroupsTypes` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
